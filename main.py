...
# ================= Q9: /solve =================
solve_history = []   # every /solve call this session: problem, raw LLM output, final answer

@app.get("/solve-history")
def get_solve_history():
    """Full history of every /solve call this session (newest first), so you can
    see the EXACT problem text the grader sent (e.g. p22) and debug why the
    answer was wrong. Open https://<your-app>.onrender.com/solve-history"""
    return {"count": len(solve_history), "calls": list(reversed(solve_history))}
...
async def solve(request: Request):
    ...
    raw_llm, ans, reasoning, err = "", None, "", None
    try:
        # Q9 is graded on exact integer correctness -> use the strongest model.
        raw_llm = await chat([{"role": "user", "content": prompt}],
                              model="gpt-4o", max_tokens=1200)
        ...
        result = {"reasoning": reasoning, "answer": ans}
    except Exception as e:
        err = str(e)
        result = {"reasoning": "Could not solve reliably: " + str(e)[:120].ljust(80),
                  "answer": 0}
